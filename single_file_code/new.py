...
class CameraStreamWorker(QThread):
    ...
    def run(self):
        self.running = True
        retry_delay = 1000  # Start with 1 second retry delay
        
        while self.running:
            try:
                # Performance improvements for OpenCV capture
                ...
                # Try to set RTSP transport to TCP (more reliable than UDP)
                cap.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*'H264'))
                
                # Attempt to get lower resolution for better performance
                ...
                frame_count = 0
                skip_frames = 1  # Process every Nth frame (adjust for performance)
                
                while self.running:
                    ret, frame = cap.read()
                    if not ret:
                        log.warning(f"Camera {self.cam_id}: Failed to read frame, reconnecting...")
                        break
                    ...
                    # 20ms = ~50fps, 33ms = ~30fps, 66ms = ~15fps
                    self.msleep(15)  # Faster refresh rate
                
                cap.release()
                
                # If we got here but still running, try to reconnect
                if self.running:
                    self.connectionStatus.emit(self.cam_id, False)
                    self.reconnect_attempts += 1
                    if self.reconnect_attempts > self.max_reconnect_attempts:
                        log.error(f"Camera {self.cam_id}: Max reconnection attempts reached")
                        break
                        
                    # Exponential backoff for reconnection attempts
                    retry_delay = min(retry_delay * 2, 10000)  # Max 10 second delay
                    self.msleep(retry_delay)
                    
            except Exception as e:
                log.error(f"Camera {self.cam_id} error: {str(e)}")
                self.connectionStatus.emit(self.cam_id, False)
                self.reconnect_attempts += 1
                if self.reconnect_attempts > self.max_reconnect_attempts:
                    log.error(f"Camera {self.cam_id}: Max reconnection attempts reached")
