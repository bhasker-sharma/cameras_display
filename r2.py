...
"""
Camera Monitoring System - Simplified Version
"""
...
class BaseApp:
    ...
    def load_logo(self):
        """Load the logo from file or create default"""
        try:
            ...
            self.logger.warning("Logo file not found, using default")
            
            # Create a default logo
            pixmap = QPixmap(160, 120)
            pixmap.fill(QColor(0, 102, 204))  # Blue
            return pixmap
    ...
    def create_transparent_logo(self):
        ...
        # Create transparent copy (30% opacity)
        temp_pixmap = QPixmap(self.logo_pixmap.size())
...
class SystemConfigDialog(QDialog):
    def __init__(self, base_app, parent=None):
        ...
        # Camera count dropdown with predefined options
        self.camera_count_combo = QComboBox()
        self.camera_count_combo.addItems(["8", "12", "16", "20", "24", "32", "40", "48"])
        
        # Set current selection based on config
        current_count = str(self.base_app.config.get("camera_count", 24))
        index = self.camera_count_combo.findText(current_count)
        if index >= 0:
            self.camera_count_combo.setCurrentIndex(index)
        else:
            # Default to 24 if current value not in list
            index = self.camera_count_combo.findText("24")
            if index >= 0:
                self.camera_count_combo.setCurrentIndex(index)
        
        form_layout.addRow("Number of Cameras:", self.camera_count_combo)
        
        main_layout.addLayout(form_layout)
        
        # Add information about window layouts
        info_label = QLabel(
            "Grid layout based on selected camera count:\n\n"
            "• 8 cameras: 4 columns × 2 rows\n"
            "• 12 cameras: 4 columns × 3 rows\n"
            "• 16 cameras: 4 columns × 4 rows\n"
            "• 20 cameras: 4 columns × 5 rows\n"
            "• 24 cameras: 4 columns × 6 rows\n"
            "• 32 cameras: Two windows with 4×4 grid each\n"
            "• 40 cameras: Two windows with 4×5 grid each\n"
            "• 48 cameras: Two windows with 4×6 grid each\n"
        )
    ...
    def save_system_config(self):
        try:
            # Update camera count
            self.base_app.config["camera_count"] = int(self.camera_count_combo.currentText())
...
# Individual camera display widget with fixed size
class CameraWidget(QWidget):
    ...
    def __init__(self, camera_number, base_app, parent=None):
        ...
        # Set FIXED size policy - this is a key change
        ...
        self.status_strip.setFixedHeight(5)  # Height of the status bar
        self.layout.addWidget(self.status_strip)
        
        # Camera display area - make it expand
        ...
        # Set logo as background for non-configured cameras
    ...
    def resizeEvent(self, event):
        ...
        if not self.stream and self.base_app.transparent_logo:
            self.display_area.setPixmap(self.base_app.transparent_logo.scaled(
                self.display_area.size(), Qt.KeepAspectRatio, Qt.SmoothTransformation)) 
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Create grid layout for cameras with center alignment
        self.grid_layout = QGridLayout()
        self.grid_layout.setSpacing(8)  # Space between cameras
        self.grid_layout.setContentsMargins(10, 10, 10, 10)  # Margins around the grid
        ...
        self.content_layout.addLayout(self.grid_layout, 1)#add stretch
    ...
    def update_camera_grid(self):
        ...
        # Get camera count
        camera_count = int(self.base_app.config.get("camera_count", 24))
        
        # Set rows and columns based on camera count
        if camera_count == 8:
            rows, cols = 2, 4
            self.create_camera_grid(rows, cols, 0, camera_count)
            ...
            # Two windows with 4x4 grid
            rows, cols = 4, 4
            self.create_camera_grid(rows, cols, 0, 16)
            self.create_second_window(rows, cols, 16, 16)
        elif camera_count == 40:
            # Two windows with 4x5 grid
            rows, cols = 5, 4
            self.create_camera_grid(rows, cols, 0, 20)
            self.create_second_window(rows, cols, 20, 20)
        elif camera_count == 48:
            # Two windows with 4x6 grid
            rows, cols = 6, 4
            self.create_camera_grid(rows, cols, 0, 24)
            self.create_second_window(rows, cols, 24, 24)
    ...
    def create_camera_grid(self, rows, cols, start_index, count):
        ...
        # Calculate widget size based on window size
        available_width = self.width() - 20  # Account for margins
        available_height = self.height() - 70  # Account for menu bar and margins
        ...
        for i in range(min(count, rows * cols)):
            ...
            camera_widget.setMinimumSize(cell_width, cell_height)
    ...
    def create_second_window(self, rows, cols, start_index, count):
        ...
        # Show maximized and bring to front
    ...
    def show_fullscreen(self, camera_number):
        ...
        existing_widget = self.camera_widgets.get(camera_number)
        if not existing_widget:
            self.logger.warning(f"No widget found for camera {camera_number}")
            return

        # Hide all other widgets
        for cam_num, widget in self.camera_widgets.items():
            if widget != existing_widget:
                widget.hide()

        # Make the selected widget expand fullscreen
        self.fullscreen_widget = existing_widget
        self.fullscreen_widget.setParent(self.content_widget)
        self.fullscreen_widget.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)

        # Remove it from grid and add to layout directly
        self.grid_layout.removeWidget(self.fullscreen_widget)
        self.content_layout.addWidget(self.fullscreen_widget)
        #self.fullscreen_widget.setFixedSize(self.content_widget.size()) #resize to fill immdidiately before
        self.fullscreen_widget.show()

        # Connect exit handler
        try:
            self.fullscreen_widget.double_clicked.disconnect()
        except:
            pass
    ...
    def exit_fullscreen(self):
        ...
        camera_number = self.fullscreen_widget.camera_number
        widget = self.fullscreen_widget

        # Remove from fullscreen layout
        self.content_layout.removeWidget(widget)

        # Add it back to the original grid layout
        camera_count = int(self.base_app.config.get("camera_count", 24))
        if camera_count in (8, 12, 16, 20, 24, 32, 40, 48):
            if camera_count <= 24:
                cols = 4
                index = camera_number - 1
            else:
                # Second screen not handled here
                return
        ...
        for cam_num, cam_widget in self.camera_widgets.items():
            cam_widget.show()
